...
std::vector <Package> packages = {
    {"RSGL","logos/RSGL.png","https://github.com/ColleagueRiley/RSGL.git","~15.0 MB","A very simple GUI library that can be used to make GUIs very easily and quickly.\nUsed to make many of the projects on this list","Colleague Riley / RSGL-Org","make install"},
...
void* installThread(void*){if (!checkIfNull(&threadP)) return NULL; std::string cmd= "cd /usr/share/RSGLApps/apps && git clone " + threadP.repo + " "+ threadP.name + " && cd " + threadP.name + " && "+ threadP.installCmd; exec(cmd.c_str()); reloadApps(); return NULL;}
void* removeThread(void*){ if (!checkIfNull(&threadP)) return NULL; std::cout << "cd /usr/share/RSGLApps/apps && rm -r " + threadP.name << std::endl; std::string cmd= "cd /usr/share/RSGLApps/apps && rm -r " + threadP.name; if (threadP.deleteCmd.size()) cmd += " && " + threadP.deleteCmd; std::cout << cmd.c_str() << std::endl; exec(cmd.c_str());  exec(cmd.c_str()); reloadApps(); return NULL;}
void* updateThread(void*){ if (!checkIfNull(&threadP)) return NULL; std::string cmd= "cd /usr/share/RSGLApps/apps/" + threadP.name + " && git pull -f && " + threadP.installCmd; exec(cmd.c_str()); reloadApps(); return NULL;}

void installPackage(Package p){threadP=p; pthread_t t; pthread_create(&t,NULL,installThread,NULL);}
void removePackage(Package p){threadP=p; pthread_t t; pthread_create(&t,NULL,removeThread,NULL);}
void updatePackage(Package p){threadP=p; pthread_t t; pthread_create(&t,NULL,updateThread,NULL);}
